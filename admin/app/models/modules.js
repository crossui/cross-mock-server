...
//查找数据
async function find({ modulename = '', projectid = ''} = {}) {
    modulename = replaceSingleQuotes(modulename)
    let sql = `
        select * from cross_module
        where modulename like '%${modulename}%'
        and projectid = '${projectid}'
        order by mid desc;
    `
    //limit ${starLimit}, ${endLimit}   , starLimit = 0, endLimit = 10 
    //let totalSql = `select count(*) from cross_module where modulename like '%${modulename}%' and projectid = '${projectid}'`
