...
class UsersCtl {
  ...
  async find(ctx) {
    ...
    const res = await User.findUser({ account, starLimit, endLimit })
  ...
  async login(ctx) {
    ...
    const repeatedUser = await User.checkUser({ account, password });
    if (repeatedUser) {
      const res = await User.findUserOne({ uid: repeatedUser[0].uid })
      if (res[0].verify == "2") {
  ...
  async update(ctx) {
    ...
    console.info(ctx.params.id, ctx.request.body)
